 ...
 * -> Random play: I adjusted the gameplay at this point to only allow users to
 * pick a random card and move
 * around if they believe they can't win with their provided card.
 * -> Manual play: User only need to enter a string of location with default
 * format so if the card has
...
public class Bingo {
     ...
     * It handles user input, ensures valid choices, and directs the gameplay
     * accordingly
     * 
    ...
    /**
     * This method allows the user to manually play the Bingo game. It displays the
     * available cards and prompts the user to pick a card by entering its ID.
     * It ensures that the user input is an integer within the valid range (1-8). If
     * the input is invalid, it prompts the user to try again.
     * 
     ...
     * @return true if all elements of the subset are present in the set, otherwise
     *         false.
     ...
     * It stores all user inputs in an ArrayList and uses a multidimensional
     * ArrayList
     * to check if the inputs are correct answers.
     * It adjusts the positions of the matrix based on the user's inputs.
     * User doesn't need to call the number to win the game, they only need to put
     * the right location to win
    ...
    public static void manualGameplay(BingoCard card, int cardID) {
        ...
        System.out
                .println(">>Program stricly follow formatting rules: row then column for your input! For example: BI\n"
    ...
    /**
     * This method allows the user to play the Bingo game. It stores all user inputs
     * in an ArrayList and
    ...
    private static boolean randomGameplay(BingoCard card, boolean isRand, BingoCaller callObject) {
        ...
        while (!flagForInput) {
            try {
                ...
                System.out.println("Your input should be something like: BB,IB,BO... ");
                System.out.println(
                        ">>Program stricly follow formatting rules: row then column for your input! For example: BI\n"
                                + "with B will be the row and I will be the column<<");
        ...
        while (!tempUsrInput.toUpperCase().equals("BINGO")) {
            // Move to another card when user believes they can't win by using their
            // provided card
            ...
            if (card.getValue(row, col).equals("XX")) {
                ...
                if (convertToIntVal != antiCheatValue) {
                    card.display();
                    System.out.println("Value doesn't match current call: " + tempAntiCheatString
                            + "! You can \"CALL\" again or \"MOVE\": ");
     ...
     * There are 12 cases to win a Bingo game so we will need to store all the cases
     * in separated lists
     * 
     * @param selectedCard
